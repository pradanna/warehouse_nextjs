...
export async function getGeneralLedger({
  ...
  try {
    ...
    const response = await axiosInstance.get(
      `${baseUrl}/summary/outlet-general-ledger?${params.toString()}`,
      {
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
    ...
    console.log("Get General Ledger Success:", response.data.data);
    return response.data;
  } catch (err) {
    console.error("Gagal mengambil data General Ledger:", err);
    throw err;
  }
}
