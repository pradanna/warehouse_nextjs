...
export const generatePurchasePDF = (data: any) => {
  const doc = new jsPDF();
  ...
  doc.text(`Supplier: ${data.data.supplier.name}`, rightX, y, {
    align: "right",
  });
  ...
  // Baris 2
  doc.text(`Tanggal Pembelian: ${data.data.date}`, leftX, y);
  ...
  //   doc.save(`Pembelian-${data.reference_number}.pdf`);
};
