...
export default function PurchaseDetailModal({
  ...
  const totalPaid = purchaseDetail.data.payments.reduce(
  ...
  const handleSavePayDebt = async () => {
    ...
    try {
      const response = await createPurchasePayment(payload);
          ...
          <div className="md:col-span-8 bg-white border border-gray-200 p-4 rounded-md">
            ...
            <div className="overflow-auto">
              <table className="w-full text-sm text-left">
                ...
                <tbody>
                  {purchaseDetail.data.items.map((item: any, index: number) => (
          ...
          <div className="md:col-span-12 bg-white border border-gray-200 p-4 rounded-md">
            <div className="flex justify-between items-center">
              <h2 className="text-md font-semibold text-gray-700 mb-2">
                Pembayaran
                <span
                  className={`inline-block text-xs font-semibold px-3 py-1 rounded-full ml-2 ${
            ...
            {purchaseDetail.data.payment_status !== "paid" && (
              <div className="flex justify-end mt-4">
                <div className="bg-yellow-100 text-yellow-800 px-4 py-2 rounded-md text-sm font-semibold">
                  Kekurangan Bayar: Rp{" "}
                  {remainingPayment.toLocaleString("id-ID")}
                </div>
              </div>
            )}
          </div>
